...
def shadow_text(dwg, x, y, text, font_size=60):
...
def generate_svg(dwg, objs, labels, text_lines):
    ...
    for obj in objs:
        ...
        shadow_text(dwg, x, y - 15, label)
	# shadow_text(dwg, x, y - 5, label)
        dwg.add(dwg.rect(insert=(x,y), size=(w, h),
                        # fill='red', fill_opacity=0.3, stroke='white'))
			fill='red', fill_opacity=0.1, stroke='red', stroke_width=15))

def main():
    default_model_dir = '../all_models'
    default_model = 'mobilenet_ssd_v2_coco_quant_postprocess_edgetpu.tflite'
    default_labels = 'coco_labels.txt'
    parser"""A demo which runs object detection on camera frames."""
